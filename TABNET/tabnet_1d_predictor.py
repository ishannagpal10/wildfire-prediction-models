...
def bayes_optim(scorer, direction):
    ...
    best_model.fit(X_train, y_train, patience=10, max_epochs=50, batch_size=1024, virtual_batch_size=128, num_workers=10)
...
y_pred = best_model.predict(X_gpr)

def adjuster(X_gpr, y_gpr, X_test, y_test, y_pred, model):
    y_gpr = y_gpr.ravel()
    y_pred = y_pred.ravel()
    y_test = y_test.ravel()

    residuals = y_gpr - y_pred
    ...
    adj_preds = model.predict(X_test).ravel() + residual_preds
...
r2_, rmse_, mae_, adj_preds = adjuster(X_gpr, y_gpr, X_test, y_test, y_pred, best_model)
print("R2 =", r2_)
print("RMSE =", rmse_)
print("MAE =", mae_)
